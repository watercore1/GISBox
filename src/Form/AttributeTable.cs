...
namespace GISBox
{
    public partial class AttributeTable : Form
    {
        ...
        MainForm father_form = new MainForm();//表示父窗口
        ...
        public AttributeTable(MainForm temp, int index)
        {
            InitializeComponent();
            select_index = index;
            layer_show = temp.mapControl.Layers.GetItem(index);
        ...
        public void Load_frame()
        {
            if (Show_select)
            {
                ...
                for (int i = 0; i < layer_show.AttributeFields.Count; i++)
                {
                    ...
                    {
                        dataTable_select.Columns.Add(layer_show.AttributeFields.GetItem(i).Name, typeof(short));
                    }
                    else if (layer_show.AttributeFields.GetItem(i).ValueType == MyMapObjects.moValueTypeConstant.dInt32)
                    {
                        dataTable_select.Columns.Add(layer_show.AttributeFields.GetItem(i).Name, typeof(int));
                    }
                    else if (layer_show.AttributeFields.GetItem(i).ValueType == MyMapObjects.moValueTypeConstant.dInt64)
                    {
                        dataTable_select.Columns.Add(layer_show.AttributeFields.GetItem(i).Name, typeof(long));
                    }
                    else if (layer_show.AttributeFields.GetItem(i).ValueType == MyMapObjects.moValueTypeConstant.dSingle)
                    {
                        dataTable_select.Columns.Add(layer_show.AttributeFields.GetItem(i).Name, typeof(float));
                    }
                ...
                for (int i = layer_show.SelectedFeatures.Count - 1; i >= 0; i--)
                {
            ...
            else
            {
                ...
                for (int i = 0; i < layer_show.AttributeFields.Count; i++)
                {
                    ...
                    {
                        dataTable.Columns.Add(layer_show.AttributeFields.GetItem(i).Name, typeof(short));
                    }
                    else if (layer_show.AttributeFields.GetItem(i).ValueType == MyMapObjects.moValueTypeConstant.dInt32)
                    {
                        dataTable.Columns.Add(layer_show.AttributeFields.GetItem(i).Name, typeof(int));
                    }
                    else if (layer_show.AttributeFields.GetItem(i).ValueType == MyMapObjects.moValueTypeConstant.dInt64)
                    {
                        dataTable.Columns.Add(layer_show.AttributeFields.GetItem(i).Name, typeof(long));
                    }
                    else if (layer_show.AttributeFields.GetItem(i).ValueType == MyMapObjects.moValueTypeConstant.dSingle)
                    {
                        dataTable.Columns.Add(layer_show.AttributeFields.GetItem(i).Name, typeof(float));
                    }
                    else if (layer_show.AttributeFields.GetItem(i).ValueType == MyMapObjects.moValueTypeConstant.dText)
                    {
        ...
        public void Add_field()
        {
            ...
            for (int i = 0; i < this.father_form.mapControl.Layers.GetItem(select_index).Features.Count; i++)
                temp_array.Add(0);
            moAttributestemp.FromArray(temp_array.ToArray());
            this.father_form.mDbfFiles[select_index].AddField(moFieldtemp, moAttributestemp);//保存进去，全是一堆零
            layer_show = this.father_form.mapControl.Layers.GetItem(select_index);
            Fieldaddsave = true;
            refresh();//重新加载一下
        ...
        /// <summary>
        /// 更新MainForm窗体内的显示,由选中行更改图标
        /// </summary>
        public void Refresh_MainFormform_select()
        {
            ...
            this.father_form.mapControl.RedrawTrackingShapes();
            Refresh_scaletext_select();//更新一波标签
        }
        ...
        private void dataGridView1_CellParsing_1(object sender, DataGridViewCellParsingEventArgs e)
        {
            ...
            this.father_form.mapControl.Layers.GetItem(select_index).Features.GetItem(row).Attributes.SetItem(col, e.Value);
            layer_show = this.father_form.mapControl.Layers.GetItem(select_index);
            Arributesave = true;
            refresh();//重新加载一下
        }
        ...
        private void dataGridView1_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            this.Refresh_MainFormform_select();
        }
        //用于操作鼠标点选或者拖动要素
        private void dataGridView1_CellMouseUp(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if (e.ColumnIndex < 0)
            {
                this.Refresh_MainFormform_select();//更新窗体显示事件
            }
            else
            {
